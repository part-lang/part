...
PHP_METHOD(Plang_Main_Plang, engine) {
	...
	zephir_check_call_status();
	ZEPHIR_CPY_WRT(plang, _16);
	ZEPHIR_CALL_METHOD(&_16, main, "switchs", NULL, 10, plang);
	zephir_check_call_status();
	ZEPHIR_CPY_WRT(plang, _16);
	ZEPHIR_INIT_NVAR(_0);
	...
	ZEPHIR_CPY_WRT(plang, _16);
	ZEPHIR_INIT_NVAR(_0);
	zephir_eval_php(plang, _0, "/home/ubuntu/workspace/plang/plang/main/plang.zep:59" TSRMLS_CC);
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Plang_Main_Plang, notfound) {
	...
	if (!((zephir_file_exists(location TSRMLS_CC) == SUCCESS))) {
		...
		ZEPHIR_CALL_FUNCTION(&_2$$3, "date", NULL, 12, &_0$$3);
		zephir_check_call_status();
