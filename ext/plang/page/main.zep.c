...
ZEPHIR_INIT_CLASS(Plang_Page_Main) {
	...
	ZEPHIR_REGISTER_CLASS(Plang\\Page, Main, plang, page_main, plang_page_main_method_entry, 0);
...
PHP_METHOD(Plang_Page_Main, notfound) {
	...
	if (!((zephir_file_exists(location TSRMLS_CC) == SUCCESS))) {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "header", &_1, 2, &_0$$3);
		zephir_check_call_status();
		ZEPHIR_SINIT_NVAR(_0$$3);
		ZVAL_STRING(&_0$$3, "D, d M Y h:i:s T", 0);
		ZEPHIR_CALL_FUNCTION(&_2$$3, "date", NULL, 3, &_0$$3);
		zephir_check_call_status();
		ZEPHIR_INIT_VAR(_3$$3);
		ZEPHIR_CONCAT_SV(_3$$3, "Date: ", _2$$3);
		ZEPHIR_CALL_FUNCTION(NULL, "header", &_1, 2, _3$$3);
		zephir_check_call_status();
		ZEPHIR_SINIT_NVAR(_0$$3);
